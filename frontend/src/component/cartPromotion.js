...
const CartPromotion = (props) => {
    ...
    const deletePromotionHandler = (cartId, promotionId) =>{
        ...
        deletePromotion({variables:{cartId:cartId, promotionsId:promotionId}}).then(
    ...
    return(
        <div className="col-lg-12 row mainnaja pr-0 pl-0 ml-0 mb-4 mt-4">
            <div className="headborder bg-dark text-light col-4">
                ...
                <h6 className="boldhead text-light">Discount {item?.forPromotion?.promotionName}</h6>
            </div>
            <div class="bg-light col-8 bodyborder">  
            <div className="row flexbetween pl-3 pr-3">
                    <h5 className="boldhead mb-1">{item?.forPromotion?.disProduct?.productName}</h5>
                     <button className="flexend btn bg-danger text-light positiondelete" onClick={() => deletePromotionHandler(dataCart?.cart?._id,item.forPromotion?._id)}>-</button>
                 </div>
                <hr/>
