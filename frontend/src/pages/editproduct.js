...
function EditOrder() {
    ...
    const {data:dataProduct, loading:loadingProduct} = useQuery(FIND_PRODUCT_QUERY, {variables: { id: name.string }})
    ...
    const onUploadImg =  async () => {
       console.log(img)
       if(isImgchange){
            const { data: { upload: filename } } = await Create({ variables : { imgUrl : img}})
            const record = {
    ...
    const Update =  async (record) => {
        await UpdateProduct({variables: {record : record,productId : dataProduct?.product?._id}}).then( (result) => {
            console.log(result)
            const productId = result.data.updateProductById.record._id
            const productQuantity = parseFloat(quantity)
            updateStock({variables: { productId : productId, quantity : productQuantity}})
           })
    }
    ...
    <div className="bg">
        <Admin/>
          ...
          <h2 className="Texttitle mt-5" data-aos="fade-right">Edit Product {dataProduct?.product._id}</h2>
          <hr/>
          <div className="row">
           <div className="col-lg-7 col-xs-12">
                  <form enctype="multipart/form-data row">
                        <label class="form-label" for="customFile">Image of product</label>
                        <input type="file" class="form-control bg-light" onChange={handleFileChange} id="customFile" required/> 
                        
                        <div class="form-group mt-2">
                            <label class="form-label" for="customFile">Product name</label>
                            <input type="text" class="form-control bg-light" onChange={(e) => setProductName(e.target.value)} placeholder="Name of product"value={productName} /> 
                        </div>
                        <div class="form-group mt-2">
                            <label class="form-label" for="customFile">Product description</label>
                            <input type="text" class="form-control bg-light" onChange={(e) => setProductDesc(e.target.value)} placeholder="description of product" value={productDesc} /> 
                        </div>
                        <div class="form-group mt-2">
                            <label class="form-label" for="customFile">Price</label>
                            <input type="number" class="form-control bg-light" min="0" max='9999' onChange={(e) => setPrice(e.target.value)} placeholder="Price of product" value={price}/> 
                        </div>
                        <div class="form-group mt-2">
                            <label class="form-label" for="customFile">Gender</label>
                            <select class="form-control bg-light" id="exampleFormControlSelect1" value={gender} onChange={(e) => {
                        ...
                        <div class="form-group mt-2">
                            ...
                            <input type="number" class="form-control bg-light" min="0" max='9999' onChange={(e) => setQuantity(e.target.value)} placeholder="Quantity of product" value={quantity}/> 
                        </div>
                        <button class="btn btn-light mt-2" type="button" onClick={onUploadImg}>Submit</button>
        ...
        <div className="col-lg-5 col-xs-12">
          ...
          <h2 className="texttitle">Preview</h2>
              <div class="card bg-light text-dark shadow shoecard">
                            <img class="card-img-top imgs" src={imgUrl== null ? notfound:imgUrl} alt="Card image cap"/>
                            <div class="card-body">
                                <h5 class="card-title">{productName== ''? 'Input name': productName}</h5>
                                <p class="card-text">{productDesc == ''? 'Input Description': productDesc}</p>   
                            </div>
                            <div class="card-footer text-dark flexbe ">
                    
                                <h6 className="boldhead mb-0 totaltext mt-2 ml-1">{price} USD</h6>
                                <div className="row mt-2">
                        
                                    <div className="col-6">
                                        
                                        <button href="#" class="btn btn-light col" disabled>more</button>
                                       
                                    </div>
                                    <div className="col-6">
                                    <button class="btn btn-dark col" disabled>Add</button>
                                    </div>
                                </div>
                            </div>
                        </div> 
