...
function PaymentPage() {
    ...
    const changeOrderStatus = () =>{
        if(
            ...
            updateOrderStatus({variables:{orderId:order._id, status:'PAID'}})
    ...
    <div className="bg">
        <Navbar/>
            <div className="container mt-5">
                <h2 className="Texttitle" data-aos="fade-right">Payment</h2>
                <hr></hr>
                <div className="row">
                    <div className="col-lg-8 col-sm-12">
                            <div class="form-group mt-2">
                                <label class="form-label" for="customFile">Card Number</label>
                                <input type="text" class="form-control bg-light" placeholder="Card Number" id="customFile" onChange={handleCardNumberChange} required/> 
                            </div>
                            <div class="form-group mt-2">
                                <label class="form-label" for="customFile">Cardholder Name</label>
                                <input type="text" class="form-control bg-light" placeholder="Cardholder Name" id="customFile" onChange={handleHolderNameChange} required/> 
                            </div>
                            <div className="row">
                                <div className="col-lg-4 col-sm-12">
                                    <div class="form-group mt-2">
                                        <label class="form-label" for="customFile">CVV</label>
                                        <input type="text" class="form-control bg-light" placeholder="CVV" id="customFile" onChange={handleCvvChange} required/> 
                                    </div>
                                </div>
                                <div className="col-lg-8 col-sm-12">
                                    <div class="form-group mt-2">
                                        <label class="form-label" for="customFile">Expire Date</label>
                                        <input type="date" class="form-control bg-light" placeholder="Expire Date" id="customFile" onChange={handleExpireDate} required/> 
                                    </div>
                                </div>
                            </div>
                            <br/>
                            <button className='btn btn-primary' onClick={() => changeOrderStatus()}> Pay </button>
                    </div>
                </div>
            </div>  
    </div>
