...
function EditPromotion() {
    ...
    const {data:dataPromotion, loading:loadingPromotion} = useQuery(FIND_PROMOTION_ONE, {variables: { promotionId: name.string }})
    ...
    useEffect(async ()=>{
        ...
        QueryProduct({variables: { id: dataPromotion?.promotion?.disProduct?._id }})
    }, [loadingPromotion])
    useEffect(async ()=>{
        setProductSelect(dataProduct)
    ...
    <div className="bg">
       <Admin/>
            <div className="container mt-5">
                ...
                <h3 className="Texttitle" data-aos="fade-right">Edit Promotion : {promotionName}</h3>
               </div>
                ...
                <div className="col-lg-12 mt-2">
                  ...
                  <div className="col-6 mt-4">
                        <div className="bg-dark boxproduct text-light" style={{backgroundImage: "url(" + productSelect?.product?.imgUrl + ")"}}>
                            <div className="filterbg" style={{boxShadow: dataProduct?.product?._id === selected? "0px 0px 0px 10px #5cb85cE0": "",transition: "0.25s"}}>
                            <p className="text-light">{dataProduct?.product?.productName? <>{dataProduct?.product?.productName}</> : <>loading...</>}{console.log(productSelect)}</p>
                            <p className="text-light">Price: {dataProduct?.product?.price} USD</p>
                            </div>
                            </div>
                    </div>
                  <hr/>
